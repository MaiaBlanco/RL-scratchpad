...
class TemporalDifference21:
    ...
    def __init__(self, N0=100, n_steps=-1):
        ...
        if n_steps == 0:
            print("Warning: n_steps must be -1 (TD-lambda) or greater than 0." \
                "Defaulting to TD-lambda.")
        ...
        while not self._game._game_over:
            ...
            for index, val in enumerate(state_actions_rewards):
                if self._lookahead == -1:
                    l = len(state_actions_rewards)
                    b = 0
                else:
                    l = self._lookahead+1
                    b = l
                for lookahead in range(b,l):
                    ...
                    # if d_score_now == -1:
                    #     print("Hello")
                    best_target_return = np.amax(Q[d_score_now, p_score_now, :])
